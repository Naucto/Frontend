...
const MODULE_COMPLETIONS = new Map<string, LuaCompletion[]>([
  ["net", [
    ...
    {
      label: "lock",
      detail: "net.lock(path)",
      documentation: "Returns a lock handle: acquire(fn) runs fn(release) as a host-serialized critical section (call release() when finished); is_locked() reports whether it is held.",
      insertText: "lock(${1:path})",
      kind: "function",
    },
    {
      label: "queue",
      detail: "net.queue(path)",
      documentation: "Returns a host-serialized FIFO queue handle: push(value), pop(callback) where callback receives the popped value (or nil when empty), length() and peek() for local reads.",
