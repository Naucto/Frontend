  ...
  ["net", [
    {
      label: "state",
      detail: "net.state",
      documentation: "The synchronized multiplayer table. Read/write fields to share state across players; the host governs writes.",
      insertText: "state",
      kind: "value",
    },
    {
      label: "host",
      detail: "net.host(onReady)",
      documentation: "Opens the host-a-session modal; calls onReady() once the session is live and you are the host.",
      insertText: "host(${1:function() end})",
      kind: "function",
    },
    {
      label: "join",
      detail: "net.join(onReady)",
      documentation: "Opens the browse / invite-code modal to join a session; calls onReady() once connected.",
      insertText: "join(${1:function() end})",
      kind: "function",
    },
    {
      label: "leave",
      detail: "net.leave()",
      documentation: "Leaves the current session.",
      insertText: "leave()",
      kind: "function",
    },
    {
    ...
    {
      label: "emit",
      detail: "net.emit(name, payload)",
      documentation: "Emits a custom event to the other players.",
      insertText: "emit(${1:name}, ${2:payload})",
      kind: "function",
    },
    {
      label: "lock",
      detail: "net.lock()",
      documentation: "Creates a mutual-exclusion lock. Assign it into net.state (e.g. net.state.gate = net.lock()) to make it shared and host-serialized, then use it from there: acquire(fn) runs fn(release) as a critical section (call release() when finished) and is_locked() reports whether it is held. Left out of net.state it is a local, single-player lock.",
      insertText: "lock()",
      kind: "function",
    },
    {
      label: "queue",
      detail: "net.queue()",
      documentation: "Creates a FIFO queue. Assign it into net.state (e.g. net.state.jobs = net.queue()) to make it shared and host-serialized, then use it from there: push(value), pop(callback) where callback receives the popped value (or nil when empty), length() and peek() for local reads. Left out of net.state it is a local, single-player queue.",
      insertText: "queue()",
      kind: "function",
    },
  ]],
