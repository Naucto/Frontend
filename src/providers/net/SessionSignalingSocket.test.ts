...
import { SessionSignalingSocket, SignalingSocketOptions } from "./SessionSignalingSocket";
...
const make = (overrides: Partial<SignalingSocketOptions> = {}): {
  frames: InboundFrame[];
  closed: boolean;
...
describe("SessionSignalingSocket", () => {
  ...
  it("closes when the ticket cannot be refreshed past expiry", async () => {
    const refreshTicket = jest.fn().mockResolvedValue(null);
